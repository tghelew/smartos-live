        ...
        if (disks.hasOwnProperty(disk)) {
            ...
            if (payload.brand === 'bhyve') {
                ...
                if (zvol.image_size) {
                    ...
                    quota += Math.ceil(Number(zvol.image_size) / 1024);
                } else if (zvol.size) {
                    // size is MiB, quota is GiB
                    quota += Math.ceil(Number(zvol.size) / 1024);
                }
                payload.quota = quota;
            }
        }
...
function buildZonecfgUpdate(vmobj, payload, log)
{
    ...
    if (brand === 'bhyve') {
        // If no bootrom specified, default to one compatible with KVM images.
        if (!payload.hasOwnProperty('bootrom')) {
            payload.bootrom = '/usr/share/bhyve/BHYVE_UEFI_CSM.fd';
        }
...
function checkHWSetup(vmobj, log, callback)
{
    ...
    if (vmobj.brand === 'bhyve') {
        // XXX-mg this should probably look at a zone aux state or similar
        callback(null, true);
        return;
    }
...
exports.start = function (uuid, extra, options, callback)
{
    ...
    bhyve_load_fields = [
        ...
        'ram',
        'resolvers',
        // What is this spice you speak of?
        'spice_opts',
        'spice_password',
        'spice_port',
        'state',
    ...
    async.series([
        function (cb) {
            ...
            if (['BHYVE', 'KVM'].indexOf(vm_type) !== -1) {
                ...
                VM.load(uuid, {
                    log: log,
                    fields: vmobj.brand === 'kvm'
                        ? kvm_load_fields : bhyve_load_fields
                }, function (error, obj) {
                    if (error) {
                        cb(error);
                        return;
